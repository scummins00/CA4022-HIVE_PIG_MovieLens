...
tag = LOAD 'output/cleaned_tags' USING PigStorage() as (userid: int, movieid: int, length: int, tags:bag{}, timestamps:bag{});
...
joined_r_and_t = FOREACH ratings_and_tags GENERATE ratings::userid AS userid, ratings::movieid AS movieid,
	ratings::rating AS rating, ratings::timestamp AS ratingtime, tag::tags AS tags, tag::length AS taglength,
	tag::timestamps AS tagtimes;
...
m_r_t = FOREACH j_m_r_t GENERATE movies::movieid AS movieid, movies::title AS title,
	  ...
	  joined_r_and_t::tags AS tags, joined_r_and_t::taglength AS taglength,
	  joined_r_and_t::tagtimes AS tagtimes;
